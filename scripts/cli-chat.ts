...
async function interactiveMode(
  ...
  setProvider: (p: 'openai' | 'gemini' | 'glm' | 'ollama') => void,
  ...
  const askQuestion = () => {
    rl.question('\n💬 Enter your query (or "exit" to quit, "help" for commands): ', async (input) => {
      ...
      if (query === 'exit' || query === 'quit' || query === 'q') {
        ...
        console.log('  provider ollama   - Use Ollama (self-hosted, e.g., gemma3:12b)');
        ...
        if (newProvider === 'openai' || newProvider === 'gemini' || newProvider === 'glm' || newProvider === 'ollama') {
          currentProvider = newProvider as 'openai' | 'gemini' | 'glm' | 'ollama';
          setProvider(currentProvider);
          console.log(`\n✅ Response AI switched to: ${currentProvider.toUpperCase()}`);
        } else {
          console.log(`\n❌ Invalid provider. Use: openai, gemini, glm, or ollama`);
        }
        askQuestion();
...
async function main() {
  ...
  const setProvider = (p: 'openai' | 'gemini' | 'glm' | 'ollama') => {
    currentProvider = p;
  };
