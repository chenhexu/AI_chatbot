...
async function main() {
  ...
  for (let i = 0; i < actualArgs.length; i++) {
    if (actualArgs[i] === '--provider' && i + 1 < actualArgs.length) {
      ...
      if (p === 'openai' || p === 'gemini' || p === 'glm') {
        provider = p as 'openai' | 'gemini' | 'glm';
  ...
  console.log(`   Response Provider: ${provider.toUpperCase()} (${provider === 'glm' ? 'GLM-4.7' : provider === 'gemini' ? 'Gemini' : 'OpenAI'})`);
  console.log(`   Background AI: ${backgroundAI.toUpperCase()} (${backgroundAI === 'glm' ? 'GLM-4.7' : 'Gemini'})\n`);
  ...
  for (let i = 0; i < allQuestions.length; i++) {
    ...
    console.log(`[${i + 1}/${allQuestions.length}] Testing: "${question}"`);
    ...
    console.log(`   ${status} ${result.timeTaken}ms - ${testResult.success ? 'Success' : 'Failed'}\n`);
  }
